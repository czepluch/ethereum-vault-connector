...
contract VaultExchangeRateMonitor is Assertion {
    ...
    function triggers() external view override {
        ...
        registerCallTrigger(this.assertionControlCollateralExchangeRateInvariant.selector, IEVC.controlCollateral.selector);
    }
    ...
    function assertionCallExchangeRateInvariant() external {
        ...
        for (uint256 i = 0; i < singleCalls.length; i++) {
            // Decode call parameters directly: (address targetContract, address onBehalfOfAccount, uint256 value, bytes data)
    ...
    function assertionControlCollateralExchangeRateInvariant() external {
        ...
        for (uint256 i = 0; i < controlCalls.length; i++) {
            // Decode control collateral parameters directly: (address targetCollateral, address onBehalfOfAccount, uint256 value, bytes data)
    ...
    function validateVaultExchangeRateInvariant(address vault) internal {
        ...
        if (postExchangeRate < preExchangeRate) {
            ...
            require(hasLegitimateBadDebt, "VaultExchangeRateMonitor: Exchange rate decreased without bad debt socialization");
        }
    }
    ...
    function getExchangeRate(address vault) internal view returns (uint256 exchangeRate) {
    ...
    function checkForBadDebtSocialization(address vault) internal returns (bool hasBadDebt) {
        ...
        for (uint256 i = 0; i < logs.length; i++) {
            ...
            if (log.emitter == vault) {
                ...
                // Check for Withdraw event (topic[0] = event signature, topic[1] = sender, topic[2] = receiver, topic[3] = owner)
